  ...
  return (
    <div className="space-y-10 pb-32 max-w-5xl mx-auto animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4">
        <div>
          <h2 className="text-4xl font-black tracking-tight text-white leading-none uppercase">Client Inbox</h2>
          <p className="text-[10px] text-gray-600 font-black uppercase tracking-[0.4em] mt-4">Communication Stream</p>
        </div>
        <div className="flex items-center space-x-3 text-blue-500 bg-blue-500/5 px-6 py-3 rounded-2xl border border-blue-500/10">
           ...
           <span className="text-[10px] font-black uppercase tracking-widest">{threads.length} Active Threads</span>
        </div>
      </div>

      <div className="space-y-4 px-4">
        {threads.length === 0 ? (
          <div className="bg-[#111827] rounded-[3rem] border border-white/5 p-20 flex flex-col items-center justify-center opacity-30 text-center">
          ...
          threads.map(({ job, latestMessage }) => (
            <div 
              ...
              className="bg-[#111827]/80 backdrop-blur-3xl p-8 rounded-[3rem] border border-white/5 hover:border-blue-500/30 hover:scale-[1.01] transition-all cursor-pointer flex items-center justify-between group shadow-xl overflow-hidden relative"
            >
              ...
              <div className="flex items-center space-x-8 flex-1 min-w-0">
                <div className="w-16 h-16 bg-[#0F172A] rounded-[1.8rem] flex items-center justify-center text-blue-500 border border-white/5 shadow-inner relative shrink-0">
                  <User size={24} />
                ...
                <div className="flex-1 min-w-0 space-y-2">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-[9px] font-black text-gray-700 uppercase tracking-widest">#{job.jobNumber}</span>
                  </div>
                  
                  <div className="flex items-center space-x-2">
                    {latestMessage.method === 'sms' ? <Smartphone size={12} className="text-blue-500" /> : <Mail size={12} className="text-blue-500" />}
                    <p className="text-sm font-medium text-gray-400 truncate italic">
              ...
              <div className="flex flex-col items-end space-y-3 ml-6 shrink-0">
                <div className="flex items-center space-x-2 text-gray-600">
                  <Clock size={12} />
                  <span className="text-[9px] font-black uppercase tracking-widest tabular-nums">{latestMessage.timestamp}</span>
                </div>
      ...
      {threads.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-4">
           <div className="bg-[#111827] p-8 rounded-[2.5rem] border border-white/5 shadow-xl">
              <p className="text-[8px] font-black text-gray-700 uppercase tracking-[0.4em] mb-4">Response Velocity</p>
              <p className="text-3xl font-black text-white leading-none tracking-tighter">4.2<span className="text-xs text-blue-500 ml-2">min</span></p>
           </div>
           <div className="bg-[#111827] p-8 rounded-[2.5rem] border border-white/5 shadow-xl">
              <p className="text-[8px] font-black text-gray-700 uppercase tracking-[0.4em] mb-4">SLA Compliance</p>
              <p className="text-3xl font-black text-white leading-none tracking-tighter">98<span className="text-xs text-green-500 ml-2">%</span></p>
           </div>
           <div className="bg-[#111827] p-8 rounded-[2.5rem] border border-white/5 shadow-xl">
              <p className="text-[8px] font-black text-gray-700 uppercase tracking-[0.4em] mb-4">Sentiment Index</p>
              <p className="text-3xl font-black text-white leading-none tracking-tighter">Elite</p>
           </div>
        </div>
      )}
    </div>
